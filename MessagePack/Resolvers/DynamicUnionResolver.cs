...
namespace MessagePack.Resolvers
{
    ...
    {
        static Dictionary<Type, ReverseUnionAttribute> _reverseUnionTypes;
        ...
        {
            _reverseUnionTypes = AppDomain.CurrentDomain.GetAssembliesSafeNonStandard().SelectMany(x => x.GetTypesSafe()).Where(x => x.GetCustomAttribute<ReverseUnionAttribute>() != null).ToDictionary(x => x, y => y.GetCustomAttribute<ReverseUnionAttribute>());
        ...
        static TypeInfo BuildType(Type type)
        {
            ...
            var selfAttrs = ti.GetCustomAttributes<UnionAttribute>();
            var reverseAttrs = _reverseUnionTypes.Where(x => x.Value.Type == type).Select(x => new UnionAttribute(x.Value.Id, x.Key));
            var unionAttrs = selfAttrs.Concat(reverseAttrs).OrderBy(x => x.Key).ToArray();
