...
namespace Yogollag
{
    ...
    public class InteractiveWorldEntitySync : InteractiveWorldEntity, IGhost
    {
        ...
        {
            get => base.InteractiveDef;
            set
            {
                base.InteractiveDef = value;
                OnPropChanged(3);
            }
        }
        ...
        public void Deserialize(NetDataReader stream)
        {
            ...
            CheckStream(stream, -1774426386);
            if ((mask & (1 << 3)) != 0)
            {
                CheckStream(stream, -1774426386);
                var has = stream.GetBool();
                InteractiveDef = !has ? default : (InteractiveDef)SyncTypesMap.GetSerializerForObjType(typeof(InteractiveDef)).Deserialize(stream);
                CheckStream(stream, -1774426386);
            }
        ...
        public bool Serialize(ref NetDataWriter stream, bool initial)
        {
            ...
            SafeguardStream(stream, 1196171199);
            bool hasAny = false;
            ...
            SafeguardStream(stream, -1774426386);
            if ((deltaMask & (1 << 3)) != 0)
            {
                SafeguardStream(stream, -1774426386);
                hasAny = true;
                if (InteractiveDef != default)
                {
                    stream.Put(true);
                    SyncTypesMap.GetSerializerForObjType(typeof(InteractiveDef)).Serialize(InteractiveDef, ref stream);
                }
                else
                {
                    stream.Put(false);
    ...
    {
        public override int NetId => 102446785;
