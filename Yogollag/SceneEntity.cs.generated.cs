...
namespace Yogollag
{
    ...
    public partial class SceneEntitySync : SceneEntity, IGhost
    {
        ...
        {
            CheckStream(stream, -894498071);
            //var hasAny = stream.GetBool();
            //if(!hasAny)
            //    return;
            var mask = stream.GetInt();
            CheckStream(stream, -1960963750);
            if ((mask & (1 << 0)) != 0)
            {
                CheckStream(stream, -1960963750);
        ...
        public bool Serialize(ref NetDataWriter stream, bool initial)
        {
            if (stream == null)
                stream = new NetDataWriter(true, 5);
            SafeguardStream(stream, -894498071);
            bool hasAny = false;
            ...
            SafeguardStream(stream, -1960963750);
            if ((deltaMask & (1 << 0)) != 0)
            {
                SafeguardStream(stream, -1960963750);
                hasAny = true;
            ...
            SafeguardStream(stream, 656951999);
            if ((deltaMask & (1 << 3)) != 0)
            {
                SafeguardStream(stream, 656951999);
                hasAny = true;
            ...
            SafeguardStream(stream, 254868755);
            if ((deltaMask & (1 << 4)) != 0)
            {
                SafeguardStream(stream, 254868755);
                hasAny = true;
